...
// Espruino XFSM Test Suite - All Requirements v2

/*
What changed
 xfsm_TestSuite_AllRequirements_v1 — Fixed IDs in Output
- Replaced function‑name parsing with an explicit **[id, fn]** registry.
- The SUMMARY and CSV now always include the **requirement ID**.
*/

// Drop‑in runner (paste over the previous "Runner" section)
  ...
  // =========================
  // Runner with explicit ID mapping
  // =========================

  function runTestWithId(id, fn) {
    try {
      var r = fn();
      if (!r || r.ok === undefined) r = { ok:false, msg:"Test returned no structured result" };
      r.id = id; // force the requirement ID we registered
      return r;
    } catch (e) {
      var msg = (e && e.message) ? e.message : (""+e);
      return { id:id, ok:false, msg:"Exception: " + msg };
    }
  }

  function runAll() {
    // Explicit mapping: [ "RequirementID", TestFunction ]
    var tests = [
    ...
    for (i = 0; i < tests.length; i++) {
      ...
      var id = pair[0];
      var fn = pair[1];
      results.push(runTestWithId(id, fn));
    }
    ...
    for (i = 0; i < results.length; i++) {
      ...
      print(status + " " + r.id + " : " + r.msg);
    }

    // CSV
    print("");
    print("=== CSV (RequirementID,Result,Message) ===");
    for (i = 0; i < results.length; i++) {
      var c = results[i];
