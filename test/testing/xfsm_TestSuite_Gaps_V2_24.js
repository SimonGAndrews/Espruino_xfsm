// xfsm_TestSuite_Gaps_V2_24.js
// Espruino XFSM Gap Tests — exercises requirements currently not met (V2_24)
...
// G1: Targetless must NOT run exit/entry (current: exit runs)
...
// G2a/G2b: Assign must apply before non-assign even if ordered after (targeted & targetless)
...
// G3: No-match should return unchanged state (pure) instead of undefined
...
// G6: Nested states must be rejected (currently only logged)
...
// G9: Service.send() should return this (chainable) — currently returns string on success
...
function runAllGaps() {
  ...
  function addSummary() {
    out.push(""); out.push("=== SUMMARY (Gaps) ===");
    for (var i=0;i<results.length;i++) {
      var r=results[i]; var status=r.skipped?"SKIP":(r.ok?"PASS":"FAIL");
  ...
  var i=0;
  function scheduleNext(){ setTimeout(function(){ runOne(); }, 0); }
  ...
  function runOne(){
    ...
    var r; try{ r=fn(); }
    catch(e){ pushResult(id,{ ok:false, msg:"Exception: "+(e&&e.message?e.message:(""+e)) }); i++; scheduleNext(); return; }
    if (r && r.__async__ && typeof r.__start__==="function") {
      var doneCalled=false; var to=setTimeout(function(){ if(doneCalled) return; doneCalled=true; pushResult(id,{ ok:false, msg:"Timeout (async)" }); i++; scheduleNext(); }, r.__timeout__|0);
      r.__start__(function(res){ if(doneCalled) return; doneCalled=true; clearTimeout(to); pushResult(id,res); i++; scheduleNext(); });
